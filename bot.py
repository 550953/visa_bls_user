...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Нажми, чтобы открыть анкету:", reply_markup=kb)
...
async def run_web():
    # Отдаём статику формы: /form/index.html и всё, что рядом
    app = web.Application()
    form_dir = os.path.join(os.path.dirname(__file__), "form")
